...
class AppVisGSP:
    ...
    def main_functions(self):
        ...
        with true_gs_col:
            if self._gs_variables:
                showing_data = self._gs_variables.data[:, selected_time]
            else:
                showing_data = None
            self._gs_draw_col_function(self._gs_variables, selected_time, showing_data)
            st.write("True Signal")
        with noizy_gs_col:
            if self._gs_variables:
                showing_data = (
                    self._gs_variables.data[:, selected_time] + self._noise_signal
                )
            else:
                showing_data = None
            self._gs_draw_col_function(self._gs_variables, selected_time, showing_data)
        ...
        with filtered_gs_col:
            if self._gs_variables:
                showing_data = self._filtered_gs
            else:
                showing_data = None
            self._gs_draw_col_function(self._gs_variables, selected_time, showing_data)
    ...
    def _load_graph_signal(self):
        ...
        if gs_path_not_exists:
            ...
            if (
                ...
                self._make_noise_signal(noise_std=0, change_noise_signal=True)
    ...
    def _gs_draw_col_function(self, gs_variables, selected_time, showing_data):
    ...
    def _gs_spectrum_draw_col_function(
        self, gs_variables, selected_time, is_noizy=False
    ):
        if gs_variables is not None and selected_time is not None:
            showing_data = self._gs_variables.normalized_data[:, selected_time]
            if is_noizy:
                showing_data = showing_data + self._noise_signal
